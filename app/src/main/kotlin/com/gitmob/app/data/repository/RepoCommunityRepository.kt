...
class RepoCommunityRepository @Inject constructor(private val api: GHApiClient) {
    suspend fun getContributors(owner: String, name: String, page: Int = 1): ApiResult<Pair<List<RepoContributor>, Boolean>> = safeCall {
        val values = api.get<List<RestContributor>>("/repos/$owner/$name/contributors?per_page=50&page=$page&anon=1")
        values.map { RepoContributor(it.login ?: it.name, it.avatarUrl, it.htmlUrl, it.contributions, it.type) } to (values.size >= 50)
    }
